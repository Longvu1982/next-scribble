...
export function pointerEventToCanvasPoint(e: React.PointerEvent, ref: A): Point {
    return {
...
export function getRandomStrings(stringArray: string[] | undefined) {
    ...
    for (let i = 0; i < 3; i++) {
        ...
        const randomString = arrayCopy.splice(randomIndex, 1)[0];
...
export const getMessageType = (answer: string, correctWord: string | undefined | null) => {
    if (!correctWord) return MessageType.NORMAL;
    const rate = stringSimilarity(answer, correctWord);
    if (rate === 1) return MessageType.CORRECT;
    else if (rate > 0.65) return MessageType.ALMOST;
