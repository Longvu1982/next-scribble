...
const formSchema = z.object({
    wordsList: z
        ...
        .regex(/^([\p{L}\s]+)(,\s*[\p{L}\s]+)*$/u, { message: "Paste in list of words separate by a comma" }),
    // code: z.string(),
});
...
const RoomSetting: FC<RoomSettingProps> = ({ currentRoom, currentPlayer, profileId }) => {
        ...
        <div className="p-4 h-full">
            {currentRoom.ownerId === currentPlayer.id ? (
                ...
                <div className="w-full h-full text-center flex items-center justify-center">
                    <span className="text-zinc-600 text-2xl animate-bounce">Host is changing the settings...</span>
                </div>
            )}
        </div>
    );
